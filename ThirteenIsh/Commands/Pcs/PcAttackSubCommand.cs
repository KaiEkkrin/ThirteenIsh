...
internal sealed class PcAttackSubCommand() : SubCommandBase("attack", "Rolls an attack with a player character.")
{
        ...
        IServiceProvider serviceProvider, CancellationToken cancellationToken)
    {
        ...
        // Get attack bonus (encounter is null for out-of-combat attacks)
        var attackBonus = characterSystem.GetAttackBonus(adventurer, null, bonus);

        var random = serviceProvider.GetRequiredService<IRandomWrapper>();
        var result = counter.Roll(adventurer, attackBonus, random, rerolls, ref dc, secondaryCounter, GameCounterRollOptions.IsAttack);
