...
[MessageHandler(MessageType = typeof(GmCombatSwitchMessage))]
internal sealed class GmCombatSwitchMessageHandler(SqlDataService dataService, DiscordService discordService,
    IServiceProvider serviceProvider) : MessageHandlerBase<GmCombatSwitchMessage>
{
    protected override async Task<bool> HandleInternalAsync(IDiscordInteraction interaction, string controlId,
        GmCombatSwitchMessage message, CancellationToken cancellationToken = default)
    {
        ...
        await result.Handle(
            ...
            async output =>
            {
                ...
                var embedBuilder = new EmbedBuilder()
                    .WithAuthor(user)
                    .WithTitle($"It is now '{message.Alias}''s turn.")
        ...
        : SyncEditOperation<EditOutput, EncounterResult>
    {
        public override EditResult<EditOutput> DoEdit(DataContext context, EncounterResult param)
        {
            ...
            return encounter.SetCurrentCombatant(alias) switch
            {
                ...
                CombatantRemoveResult.IsTheirTurn => CreateError($"It is already '{alias}''s turn."),
